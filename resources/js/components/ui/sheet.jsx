...
const SheetContent = React.forwardRef(function SheetContent(
  ...
  const sideClass =
    side === 'left'
      ? 'left-0 h-dvh w-[82%] max-w-sm'
      : side === 'right'
        ? 'right-0 h-dvh w-[82%] max-w-sm'
  ...
  return (
    <SheetPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          'fixed top-0 z-50 bg-[hsl(var(--background))] text-[hsl(var(--foreground))] ring-1 ring-[hsl(var(--border))] data-[state=open]:animate-in data-[state=closed]:animate-out',
          sideClass,
          className
        )}
        {...props}
