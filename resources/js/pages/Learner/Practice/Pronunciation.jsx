 ...
 * compare-only) is surfaced up front. UI-phase: recording is simulated.
 */
    ...
    >
      <div className="mt-2 space-y-4">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          <Chip selected={mode === 'word'} onClick={() => { setMode('word'); setScored(false); }}>শব্দ</Chip>
          <Chip selected={mode === 'sentence'} onClick={() => { setMode('sentence'); setScored(false); }}>বাক্য</Chip>
          <Chip selected={mode === 'pairs'} onClick={() => { setMode('pairs'); setScored(false); }}>কঠিন জোড়া</Chip>
        </div>

        {scored ? (
          <ScoredState target={target} words={MODES[mode].words} onRetry={() => setScored(false)} />
        ) : (
          <>
            {/* Target card */}
            <div className="rounded-[14px] bg-white p-5 text-center shadow-[0px_4px_12px_rgba(20,23,43,0.04)]">
              ...
              <p className="mt-1 text-[13px] text-learn-muted">{MODES[mode].phonetic}</p>
