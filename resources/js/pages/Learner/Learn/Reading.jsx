...
export default function Reading() {
  ...
  return (
    <LearnerShell title="রিডিং প্র্যাকটিস" showBack>
      <div className="mt-2">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          ...
          <Chip selected={filter === 'read'} onClick={() => setFilter('read')}>পড়া হয়েছে</Chip>
        </div>
            ...
            >
              ...
              <div className="mt-2.5 flex flex-wrap items-center gap-2 text-[13px] text-learn-muted">
                ...
                <span className="flex items-center gap-1.5">
                  ...
                  {toBnDigits(p.words)} শব্দ
                </span>
                {p.completed ? (
                  <span className="flex items-center gap-1.5">
                    <span className="size-0.5 rounded-full bg-learn-muted" />
                    <StatusChip tone="green">সম্পন্ন — {toBnDigits(p.correct)}/৫</StatusChip>
                  </span>
                ) : (
                  <span className="flex items-center gap-1.5">
                    <span className="size-0.5 rounded-full bg-learn-muted" />
                    {toBnDigits(p.minutes)} মিনিট
                  </span>
                )}
              </div>
            </Link>
        ...
        <p className="mt-6 text-center text-[13px] text-learn-muted">সব পাঠ্য অফলাইনে পড়া যাবে</p>
      </div>
    </LearnerShell>
  );
}
